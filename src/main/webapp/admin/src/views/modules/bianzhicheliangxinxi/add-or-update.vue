<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="车牌照号" prop="chepaizhaohao">
          <el-input v-model="ruleForm.chepaizhaohao" 
              placeholder="车牌照号" clearable  :readonly="ro.chepaizhaohao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="车牌照号" prop="chepaizhaohao">
              <el-input v-model="ruleForm.chepaizhaohao" 
                placeholder="车牌照号" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="机构性质" prop="jigouxingzhi">
          <el-select v-model="ruleForm.jigouxingzhi" placeholder="请选择机构性质">
            <el-option
                v-for="(item,index) in jigouxingzhiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="机构性质" prop="jigouxingzhi">
	      <el-input v-model="ruleForm.jigouxingzhi"
                placeholder="机构性质" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="编制日期" prop="bianzhiriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.bianzhiriqi" 
                type="date"
                placeholder="编制日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.bianzhiriqi" label="编制日期" prop="bianzhiriqi">
              <el-input v-model="ruleForm.bianzhiriqi" 
                placeholder="编制日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='chepaizhaohao'){
            this.ruleForm.chepaizhaohao = obj[o];
	    this.ro.chepaizhaohao = true;
            continue;
          }
	    ...
	    this.ro.cheliangzhaopian = true;
          ...
          if(o=='jigouxingzhi'){
            this.ruleForm.jigouxingzhi = obj[o];
	    this.ro.jigouxingzhi = true;
            continue;
          }
	    ...
	    this.ro.danweimingcheng = true;
          ...
          if(o=='bianzhiriqi'){
            this.ruleForm.bianzhiriqi = obj[o];
	    this.ro.bianzhiriqi = true;
            continue;
          }
        }
      }
    ...
    info(id) {
      this.$http({
        url: `bianzhicheliangxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `bianzhicheliangxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.bianzhicheliangxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
