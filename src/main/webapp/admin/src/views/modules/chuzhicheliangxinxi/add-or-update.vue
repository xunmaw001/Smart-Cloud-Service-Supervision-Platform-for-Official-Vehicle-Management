<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="处置日期" prop="chuzhiriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.chuzhiriqi" 
                type="date"
                placeholder="处置日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.chuzhiriqi" label="处置日期" prop="chuzhiriqi">
              <el-input v-model="ruleForm.chuzhiriqi" 
                placeholder="处置日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.danweimingcheng = true;
          ...
          if(o=='chuzhiriqi'){
            this.ruleForm.chuzhiriqi = obj[o];
	    this.ro.chuzhiriqi = true;
            continue;
          }
        }
      }
    ...
    info(id) {
      this.$http({
        url: `chuzhicheliangxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `chuzhicheliangxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.chuzhicheliangxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
